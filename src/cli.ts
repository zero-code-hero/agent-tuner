...
import { generateRules, generateRulesFromGaps } from "./generate.js";
...
program
  ...
  .option("--once", "Single pass (no loop) — just scan and generate")
  ...
  .action(async (repo: string, opts) => {
    ...
    if (opts.once) {
      ...
      const { info, depthAnalysis } = discoverAtDepth(path, depth, state);
      printDiscovery(info);
      const rules = generateRules(info);
      console.log(`📝 Generated ${rules.length} candidate rules\n`);
      console.log(`🧠 Scoring with ${opts.model}...`);
      const scored = await scoreRules(info, depthAnalysis, rules, state, opts.model, opts.verbose, opts.baseUrl);
      const kept = filterRules(scored, parseFloat(opts.threshold));
      let result = consolidate(kept);
      if (opts.merge) {
        const existing = info.existingAgentsMd;
        if (existing) {
      ...
      outputResult(result, outputPath, opts.dryRun);
      return;
    }
    ...
    for (let iter = state.currentIteration; iter < maxIterations; iter++) {
      ...
      if (gaps.length > 0) {
        ...
        const scored = await scoreRules(info, depthAnalysis, newRules, state, opts.model, opts.verbose, opts.baseUrl);
        const kept = filterRules(scored, parseFloat(opts.threshold));

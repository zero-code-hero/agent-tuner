...
// Strip embedded markdown headers/bullets so consolidate() owns all formatting
...
export function filterRules(scored: ScoredRule[], threshold: number = 6): KeptRule[] {
  return scored
    ...
    .map((s) => ({
      ...
      content: cleanContent(s.suggestion || s.original.content),
      score: s.score,
      reason: s.reason,
    }));
}
...
export function consolidate(kept: KeptRule[]): string {
  ...
  for (const r of kept) {
    ...
    sections[r.category].push(r.content);
  }
  ...
  function emitCategory(cat: string) {
